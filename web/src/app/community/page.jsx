    ...
    return (
        <div>
            ...
            {events.map((event, index) => (
                <div key={index} style={{ margin: "20px 0", padding: "10px", border: "1px solid #ccc", borderRadius: "8px" }}>
                    <h2>{event.title}</h2>
                    ...
                    <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                        {event.images.map((image, imgIndex) => (
                            <div key={imgIndex} style={{ position: "relative", width: "150px", height: "100px" }}>
                                <Image src={image} alt={`${event.title} image ${imgIndex + 1}`} layout="fill" objectFit="cover" />
                            </div>
                        ))}
                    </div>
